...
4. 当前训练序列搜索还不是实时批处理状态机，所以先用离线多窗口扫描验证整段结构。
```
...
当前全文件扫描仍然是离线多窗口实验，不是实时处理器：

```text
1. 窗口之间只做事件去重和 session 重建，尚未实现跨批次 MAC 上下文状态。
2. TCH/VOICE 不解码，只输出 TCH candidate 元数据。
3. SCH/F 当前会尝试解码，未通过校验时保留为 TCH candidate。
4. 后续实时处理会采用批处理方式，例如采集约 1 s 信号后再统一处理。
```

实时阶段不计划做逐 sample 的流式状态机，而是实现 batch-oriented DMO context。
这个上下文应在批次之间保存最近的 FN/TN、DCC、source/destination、MNI、当前
session、以及已输出事件的去重键。这样某个 1 s 批次里没有新的 DSB 时，仍可用
上一批留下的 DCC 和 session 上下文解释后续 DNB/STCH。

该 batch-state 机制等进入实时处理阶段时统一设计和接入；当前离线 DMO 文件解码
阶段暂不考虑。
