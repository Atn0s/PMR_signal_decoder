...
`bits_preview.txt` to the selected output directory. The current interactive
default is `ShowFigures=true` and `SaveFigures=false`, so the example opens ten
processing-stage figure windows and does not save PNG files by default.
