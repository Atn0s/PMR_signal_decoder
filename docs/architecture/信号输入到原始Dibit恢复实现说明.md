...
3. `radio_parallel_frontend` 的 Preview、频谱点击和 Run 起点，以及已居中、指定频偏与宽带自动发现；
...
本文只描述原生 MATLAB 并行路径。早期迁移用的 Python 解码桥和串行调度已删除，
它们不再是当前实现的一部分。

## 2. 当前实现的总体数据流

当前工程并非只有一条输入链。不同入口最终都把一个已选出的单信道复基带交给五种制式
各自的物理层恢复器。本文重点说明当前主入口 `radio_parallel_frontend.m` 的默认异步链路；
离线并行入口和自动 WOLA/PFB 链保留在后续章节作为对照。
...
BVSP / RAW IQ / stereo IQ WAV / memory IQ / future SDR IqChunk
       ...
       已居中或给定频偏    人工频谱点击选频      宽带自动发现
          radio.scanFile   radio_parallel_frontend  wideband WOLA/PFB
             |                |                |
...
| 已知载频过渡链/交互前端 | `ExecutionMode='tuned-parallel'` 或 `radio_parallel_frontend` | 宽带 IQ 中给出 1～5 个载频 | 融合有状态数字下变频和抗混叠抽取 | 先到 120/125 kS/s，再转 48/72 kS/s |
| 宽带自动发现 | `ExecutionMode='wideband'` | 宽带 RAW/WAV 或内存 IQ | 2 倍过采样 WOLA/PFB、候选跟踪、细 DDC | 61.44 MS/s 输入时子带为 120 kS/s，再转 48/72 kS/s |

交互式 `radio_parallel_frontend` 与自动 `radio.wideband` 是两条不同链：前者由用户在实时
绘制的频谱上选择最多五个载频，后者由 PFB 和候选检测器自动维护多个频率轨迹。当前
交互前端不会在后台同时运行 PFB 自动发现。`radio.scanFile` 也不做执行方式的
自动改写：已居中基带使用 `parallel`，已知非零频偏必须显式使用
`tuned-parallel`，自动发现使用 `wideband`。

### 2.2 `radio_parallel_frontend` 从点击到 dibit 的完整调用链

该入口只实现共享环、融合 DDC 和 process pool 并行主链，不提供串行或
client IQ 中继回退：

```text
Preview 1x
  radio_parallel_frontend/startPreview
    -> parallelSessionStart
       +-> acquireParallelPool(5 个协议 worker + 1 个 DDC worker)
       +-> prewarmProtocolWorkers / prewarmClientRuntime
       +-> sharedIqRingCreate(CI16，默认约 2 s)
       +-> spectrumActorStart(backgroundPool，best effort)
       +-> ddcActorStart(process worker，融合矩阵 DDC)
       +-> fileProducerStart(backgroundPool，20 ms/块)
           -> fileProducerAttachSpectrum
    -> parallelSessionCommand('run')
       -> fileProducerWorker(1×节拍)
          +-> sharedIqRingWriteTransport
          +-> spectrumActorWorker
...
1. 点击执行的是 `onSpectrumClick -> selectFrequency`，只更新 `state.selections` 并启用
...
BVSP 支持集中在 `radio.tuned`、`radio.replay` 和
...
`fileLoopSourceRead` 本身是 pull-based。`radio_parallel_frontend` 由 `fileProducerWorker`
在 `backgroundPool` 中按 1× 时钟主动拉取，
...
`ExecutionMode='parallel'` 要求 `FreqList=[]`。`scanBasebandIqEpochs` 先分割
RF Epoch，再并行识别制式。选出赢家后，`decodeProtocolWindow` 只调用该制式的
前端和解码器：四种 4FSK 转换到 48 kS/s，TETRA 窗口转换到 72 kS/s。
...
`ExecutionMode='tuned-parallel'` 要求 `FreqList` 仅含一个相对频偏。
`radio.tuned` 使用有状态 NCO、抗混叠滤波和整数抽取得到单信道基带，
再进入同一套 RF Epoch 和并行协议竞速。

`ExecutionMode='wideband'` 使用 WOLA/PFB 做粗信道化，候选跟踪器再做细频点
维护。`radio.psdBlindSearch` 仅是该宽带链中的局部候选辅助函数，不再对外形成
另一条离线扫描入口。

### 4.3 实时频谱点击选频

`radio_parallel_frontend` 的默认预览链由 `fileProducerWorker` 直接把每个宽带块送给后台
...
`selectFrequency(..., true, bandwidthHz)`。`radio.scope.refineCarrier` 在点击点附近搜索。
默认搜索半径为
...
`radio_parallel_frontend` 默认为 `false`，保留调试阶段的每次 PDU 上报。完成后 `lockedDecoderInit` 用追赶末端作为
...
DMR 和 P25 的整窗前端参数相吂。并行 Probe 把各候选发到不同 worker，
因此不跨 worker 共享鉴频结果。
...
离线 TETRA 窗口扫描使用的 `tetra.scanIqWindows` 可以检测多个活动段：默认前保护 50 ms、
...
`radio.scanFile` 的对外输出仍是 PDU；各制式的 dibit 位于协议包内部，并被
...
`parallelSessionStart` 在 Preview 前预热进程池、五制式和 DDC Actor。这会把大部分
...
2. `radio_parallel_frontend` 默认 20 ms、1× 异步文件生产，约 2 s CI16 共享 IQ 环；
...
8. TETRA 的自动发现仅在宽带 PFB 路径中进行；已居中或已知载频路径则由协议竞速尝试 TETRA；
...
异步生产、协议进程池和持久 Actor 需要 Parallel Computing Toolbox。并行池不可用、
数量不足或类型不匹配时直接报错。
...
- `tests.runRealtimeFrontendPhase1/2`：循环源与频谱/精调基础模块；
- `tests.runParallelFrontendLifecycle/runParallelFrontendSeparation`：隐藏 UI、共享环、
  生产/频谱/解码隔离和 Clear/二次 Run；
- `tests.runFiveSignal2p5MHzAcceptance`：2.5 MS/s、五载频近同步功能与软实时压力验收；

这些测试既包含纯合成向量，也会在外部样本存在时运行 DMR、P25、dPMR、NXDN 和 TETRA
实录路径。五路压力验收没有放进每次 `runAll`，因为它依赖大文件、
较长墙钟时间和本机并行调度环境。
