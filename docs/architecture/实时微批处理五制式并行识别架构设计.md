...
> 历史设计记录：本文保留并行化过程和测量数据。文中的串行基线、
> 串行降级和兼容执行方式已于 2026-07-22 删除，不再表示当前架构。

状态：并行化项目的设计基线；流式骨架、五制式竞速、多载波 DDC、Epoch、持久
Worker、生产/频谱/DDC 共享环形缓冲分离以及五制式原生增量解码已在开发分支实现，
串行路径在当时仍作为回归基线。

初始日期：2026-07-10；当前实现状态更新：2026-07-16
...
截至 2026-07-14，这一外层宽带结构已开始实现：新增 `radio.wideband` 的连续 2x
WOLA/PFB、粗/细候选检测、CandidateTracker、细 DDC 和每候选 RaceCoordinator。
默认 61.44 MS/s 输入形成 1024 个、间隔 60 kHz、输出 120 kS/s 的过采样粗子带。
详细设计与性能边界见
[60MHz宽带实时IQ信道化与并行解码设计](60MHz宽带实时IQ信道化与并行解码设计.md)，
阶段实现见[宽带流式前端阶段一实现记录](../records/frontend/宽带流式前端阶段一实现记录.md)。
...
本项目不增加跨 Epoch 的 Session 聚合层。相同发射机在静默关闭后重新出现时，仍以
两个独立 Epoch 上报；协议解码器内部已有的呼叫摘要 PDU 保持不变，但调度层不通过
发射机 ID、时间间隔等规则合并 Epoch。
...
截至 2026-07-13，阶段 2 的 Probe 契约、五协议兼容适配器和串行竞速器已经实现。
适配器暂时复用现有整窗前端和解码器，窗口不足时返回 `pending`，取得弱证据时返回
`candidate`，强校验通过时返回 `confirmed`，达到最大窗口仍未确认时返回
`rejected`。多协议同时确认统一返回 `ambiguous`，不会选择最快返回者。

真实样本的 5×5 单次交叉验证结果如下；每行只有信号真实制式得到强确认：

| 输入 | DMR Probe | P25 Probe | dPMR Probe | NXDN Probe | TETRA Probe | 竞速结论 |
|---|---|---|---|---|---|---|
| DMR | confirmed | rejected | no evidence | rejected（仅 LICH 弱证据） | rejected | DMR |
| P25 | no evidence | confirmed | no evidence | rejected | rejected | P25 |
| dPMR | rejected | rejected | confirmed | rejected | rejected（仅训练序列弱证据） | dPMR |
| NXDN | no evidence | rejected | no evidence | confirmed | rejected | NXDN |
| TETRA | rejected | rejected | rejected | rejected | confirmed | TETRA |

表中的 `no evidence` 是一次兼容适配器调用的中间状态；调度器会继续扩大窗口，并在
最大窗口到达后转为 `rejected`。阶段 3 仍需用起点、SNR、频偏和截断扫描统计最终
窗口，当前窗口值只是研究初值。
...
截至 2026-07-13，已实现 `radio.stream.characterizeProbeWindows`，支持起点、候选
窗口、相对附加噪声、频偏和多随机种子扫描，并输出逐条件最小确认窗口及 P50/P95/
P99。首轮五种真实样本均完成起点和 ±2 kHz 频偏验证，详细数据见
[五制式 Probe 窗口初始表征](../records/validation/五制式Probe窗口初始表征.md)。当前结果用于阶段 4 的
第一版参数，不视为生产参数冻结；增加样本和完成增量解码后必须重新测量。
...
截至 2026-07-13，阶段 4 已实现持久 process pool、异步 Start/Poll/Collect、尽力
取消、串行回退、迟到结果过滤和 AMBIGUOUS 汇总。真实 P25 的 1 秒渐进窗口测试中，
串行为 6.749 秒，冷并行为 4.381 秒，同一持久池热并行为 1.553 秒，逐 Probe 结果
一致；预热顺序不同的全量回归也出现过串行 1.133 秒、热并行 1.574 秒，因此不能把
单次加速作为默认切换依据。thread pool 因当前 DSP 使用受限 MEX 而不可用。实现和性能边界详见
...
截至 2026-07-13，已实现 Probe/赢家共用窗口解码、pre-trigger 快照、异步追赶、实时
边沿二次追赶、PDU 绝对源采样标注和 RaceCoordinator。真实 DMR 与“追赶期间继续
采集”测试通过。当前仍是整窗重算兼容层，详细限制见
...
截至 2026-07-13，阶段 6a 已实现统一 DecoderState、按协议有界回看、跨窗口 PDU
账本、健康结果和 LOCKED 后持续处理，五种真实样本通过追加 Chunk 测试。内部仍是
整窗重算，真正的因果 FIR/重采样/鉴频/符号与帧状态尚待按 NXDN/P25 优先顺序替换，
不能将 6a 标记为完整阶段 6。详见[持续解码重叠适配层](../records/recognition/持续解码重叠适配层.md)。

截至 2026-07-16，阶段 6b 已完成 NXDN96、P25、DMR、dPMR 和 TETRA DMO
的原生 `streamInit/streamDecodeChunk/streamFlush`。五种已支持模式跨 Chunk 保存
重采样、FIR/匹配滤波、鉴频或差分相位、同步游标、滚动解调/比特缓存以及协议帧/
会话状态，只处理新 IQ；锁定路径均已使用持久进程 Actor。因此当前直通/
DMO 支持范围内的阶段 6 已关闭。TETRA TMO、NXDN48/Type-D 不在此结论中；
五载波 250 ms 软实时长尾和长时间 backlog 验收仍属于调度/性能阶段。实现、
性能数据和五路压力边界见
...
截至 2026-07-14，阶段 7a 已实现 LOCKED 健康迟滞、LOSS_PENDING、强证据恢复、
仍有能量时的 RECLASSIFYING 和 generation 隔离；阶段 7b 的 RF 静默 Epoch 分段、
独立 PDU 归属、关闭上报和协议切换 Epoch rollover 已实现。相同发射机跨静默边界
不合并。UNKNOWN 指数退避和 AMBIGUOUS 二次消歧仍待后续完成。详见
[解码健康与重新分类实现记录](../records/recognition/解码健康与重新分类实现记录.md)和
...
## 26. MATLAB 离线实时前端的最终执行边界（2026-07-16）

2.5 MS/s 文件回放采用三执行域，而不是由 UI timer 串行完成全部工作：

```text
生产域：background file actor
  ├─ 可丢显示块 -> background PSD actor
  └─ 不可丢 CI16 shared ring -> reserved process DDC actor

协调域：MATLAB client
  └─ 消费已抽取基带、更新 Epoch/Race 状态、提交和轮询 Future

协议域：5 process workers
  └─ Probe / winner catch-up / persistent locked decoder
```

宽带 IQ 不经过 client。client 只接收 125 kS/s 级别的各载波基带结果与控制结果；因此
协议首次提交或结果合并出现短时尖峰时，生产者、PSD 计算和 DDC 仍独立前进。共享环形
缓冲使用固定槽、绝对样点和提交序号，覆盖未消费数据时明确报错，不能自动跳到最新块。

这一结构是当前 MATLAB 单会话能够达到的分离边界。实测 `backgroundPool` 既不能直接
取得 client process pool，也不能接收序列化的 pool 对象；跨 worker 的
`PollableDataQueue` 也不能用作 background-to-process 数据通道。因此协调域仍留在
client，UI 可能在一次 Future/actor 提交期间延迟刷新，但显示计算和输入获取不受其
阻塞。若要求 UI、协调器和解码器在操作系统线程/进程层面完全互不影响，应在 C++ 迁移
时使用 SPSC IQ ring、独立 UI 消息队列和统一任务池，而不是继续在 MATLAB 内增加 timer。

当前默认资源为 5 个协议进程加 1 个 DDC 进程；生产和 PSD 使用后台线程。SPS 与各协议
目标采样率在本阶段保持不变。具体实现、队列语义和 2.5 MHz 五路结果见
