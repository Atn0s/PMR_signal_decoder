...
switch backend
    case {'python', 'compat', 'py'}
        if any(strcmp(enabledProtocols, 'NXDN'))
            error('radio:decodeNarrowband:NxdnPythonUnsupported', ...
                ['NXDN is implemented only by the native MATLAB backend; ', ...
                 'use DecoderBackend=''matlab''.']);
        end
