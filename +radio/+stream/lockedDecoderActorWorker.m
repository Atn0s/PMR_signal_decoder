...
while true
    ...
    try
        input = message.input;
        if isfield(input, 'actorHistoryIq')
            historyChunk = struct('iq', state.incremental.historyIq);
            historyChunk = radio.stream.lockedDecoderActorUnpackChunk( ...
                historyChunk, input.actorHistoryIq);
            state.incremental.historyIq = historyChunk.iq;
            input = rmfield(input, 'actorHistoryIq');
        end
        ...
        if isfield(output, 'diagnostics')
            output.diagnostics = ...
                radio.stream.compactDecoderDiagnostics(output.diagnostics);
        end
        send(outputQueue, struct( ...
            'type', 'result', ...
            'actorId', uint64(actorId), ...
