function cfg = defaultConfig()
%DEFAULTCONFIG Initial configuration for the streaming scanner pipeline.
cfg = struct();
cfg.chunkDurationSec = 0.100;
cfg.ringBufferSec = 8.0;
cfg.preTriggerSec = 0.5;
cfg.lockedSuspectWindows = 3;
cfg.lockedLostWindows = 6;

cfg.activity = struct();
% NaN means that the first usable IQ chunk bootstraps the channel noise
% floor from its unoccupied FFT bins.  A finite value remains available as
% an optional receiver-provided prior (for example from a calibrated SDR).
cfg.activity.initialNoiseFloorDb = NaN;
cfg.activity.onMarginDb = 10.0;
cfg.activity.offMarginDb = 6.0;
cfg.activity.minOnSec = 0.050;
cfg.activity.offHangSec = 0.300;
cfg.activity.noiseUpdateAlpha = 0.05;
cfg.activity.minPowerDb = -160.0;
cfg.activity.spectralNoiseEnabled = true;
cfg.activity.noiseFftLength = 2048;
cfg.activity.noiseMaxSegments = 4;
cfg.activity.noiseMinSamples = 64;
end
