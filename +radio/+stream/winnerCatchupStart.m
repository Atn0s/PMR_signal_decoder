...
transferBuffer = compactTransferBuffer( ...
    buffer, epoch, p.Results.PreTriggerSec);
handle.future = parfeval(pool, @radio.stream.winnerCatchup, 1, ...
    transferBuffer, epoch, protocol, ...
    'PreTriggerSec', p.Results.PreTriggerSec, ...
    'Deduplicate', p.Results.Deduplicate);
end

function compact = compactTransferBuffer(buffer, epoch, preTriggerSec)
...
capacitySec = max(1 / buffer.sampleRateHz, ...
    numel(snapshot.iq) / buffer.sampleRateHz);
compact = radio.stream.ringBufferInit( ...
    buffer.sampleRateHz, capacitySec, ...
    'ChannelId', buffer.channelId, ...
    'CenterFrequencyHz', buffer.centerFrequencyHz);
[compact, ~] = radio.stream.ringBufferPush(compact, snapshot);
end
