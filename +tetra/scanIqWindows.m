function result = scanIqWindows(iq, sampleRate, varargin)
%SCANIQWINDOWS TETRA-only full-IQ multi-window DMO control scan.
p = inputParser;
p.addParameter('SourcePath', '');
...
if isempty(sampleRate)
    error('tetra:scanIqWindows:MissingSampleRate', 'sampleRate is required.');
end
iq = iq(:);
iq = iq - mean(iq);
...
result = struct();
result.path = char(p.Results.SourcePath);
result.inputSampleRateHz = sampleRate;
